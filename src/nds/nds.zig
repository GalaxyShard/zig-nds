...
pub fn bgGetGfxPtr(arg_id: c_int) callconv(.C) [*]u16 {
    var id = arg_id;
    _ = &id;
    if (bgState[@as(c_uint, @intCast(id))].type < BgType_Bmp8) {
        if (id < 4) {
            return @ptrFromInt(bgGetTileBase(id) * 16384 + 100663296);
        } else {
            return @ptrFromInt(bgGetTileBase(id) * 16384 + 102760448);
        }
    } else {
        if (id < 4) {
            return @as([*]u16, @ptrFromInt(@as(c_int, 100663296))) + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 8192) * bgGetMapBase(id)))));
        }
        else {
            return @as([*]u16, @ptrFromInt(@as(c_int, 102760448))) + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 8192) * bgGetMapBase(id)))));
        }
    }
}
